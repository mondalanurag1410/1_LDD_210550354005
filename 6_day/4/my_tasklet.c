...
void wrk_fn(struct work_struct *work);

struct workqueue_struct *workqueue; //make a workqueue structure
struct work_struct work; // making the work structure
...
ssize_t my_read_fn(struct file *file_d, char __user *u_buff, size_t count, loff_t *offp)
{
	ssize_t amtdata;
	int i =0;
	INIT_WORK(&work,wrk_fn); // initialize the work
	queue_work(workqueue, &work); // put work with the work queue
	while(k_buff[i] != '\0') 
		i++;
...
static int my_driver_mod_init(void)
{
	...
	int data = 0;
	workqueue = create_singlethread_workqueue("My workqueue"); // initialize the tasklet
	INIT_WORK(&work, wrk_fn); // initialize work
	queue_work(workqueue,&work); // put work within the workqueue
	return 0;
}
//////////////////////////////////////////////////////////////////module exit fuction
